...
/** Etiqueta sidebar/cabecera catálogo — siempre MAYÚSCULAS (paridad Report). */
export function labelMarcaCatalogo(descp: string | null | undefined): string {
  const k = normalizarMarcaKey(String(descp ?? ''))
  return k || '—'
}
