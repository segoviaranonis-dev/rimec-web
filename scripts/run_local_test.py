...
def run():
    print("=== ORCHESTRATING LOCAL SMOKE TEST ===")
    ...
    try:
        ...
        # 2. Run Playwright local smoke test
        print("Launching Playwright smoke test script...")
        # Use full path to node or run via npx/node
        res = subprocess.run(["node", "scripts/run_smoke_tests_local.js"], capture_output=True, text=True)
