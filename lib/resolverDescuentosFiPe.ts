 ...
 *
 * Ley Director 2026-08-10: el usuario siempre controla D1–D4.
 * LPC03 10% = sugerencia al vacío (Guido), nunca bloqueo ni pisa edición.
 */
 ...
 * · LPC03 vacío/solo-comisión: sugiere D1=10% + D2=dictado (editable; no impone).
 * · LPC03 con valores comerciales: respeta D1–D4; solo rellena D2 si vacío/basura + hay dictado.
 * · LPC03 + PROMOCIONAL: sin +10 % · D1=dictado (igual que LPN)
 * · resto: D1=dictado comercial
 * · pre_autorizado → no tocar si hay descuento comercial real (edición vendedor)
 */
export function resolverDescuentosFiPe(input: {
  ...
  // LPC03 normal: sugerencia Guido solo si vacío; nunca pisar edición del usuario
  if (lista === 3 && !promo) {
    if (sum === 0 || soloComision) {
      return [10, dictado ?? 0, 0, 0]
    }
    const d1 = Number(prev[0]) || 0
    const d2 = Number(prev[1]) || 0
    const d3 = Number(prev[2]) || 0
    const d4 = Number(prev[3]) || 0
    const d2EsBasura = d2 === 0 || COMISION_D1.has(d2)
    if (dictado != null && d2EsBasura) {
      return [d1, dictado, d3, d4]
    }
    return prev
  }
