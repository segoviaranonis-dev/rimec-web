...
import { fetchControlStockPe } from '@/lib/controlStock/fetchControlPe'
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const origen = sp.get('origen') ?? 'compra_previa'
    const soloSaldo = sp.get('solo_saldo') === '1'

    if (origen === 'pronta_entrega') {
      const depositos = sp.get('depositos')?.split('|').filter(Boolean)
      const marcas = sp.get('marcas')?.split('|').filter(Boolean)
      const estilos = sp.get('estilos')?.split('|').filter(Boolean)
      const data = await fetchControlStockPe({ depositos, marcas, estilos, soloSaldo })
